...
const FORM_INICIAL = { nombre: '', apellido: '', dni: '', telefono: '', email: '', fecha_nacimiento: '' }
...
export default function Profesores() {
  ...
  async function handleSubmit(e) {
    ...
    setGuardando(true)
    try {
      const payload = { nombre: form.nombre.trim(), apellido: form.apellido.trim(), dni: form.dni.trim(),
    ...
    } catch { setErrores({ general: 'No se pudo guardar el profesor' }) }
  ...
  return (
    <div className="space-y-5">
      ...
      <Table>
        ...
        <Tbody>
          {profesores.length === 0 ? (
            <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-400">No hay profesores registrados</td></tr>
          ) : profesores.map((p) => (
