  ...
  async function handleRegistrarPago(e) {
    ...
    try {
      const resultado = await registrarPago(cuotaSeleccionada.id, {
        monto_pagado: Number(formPago.monto_pagado), metodo: formPago.metodo, fecha_pago: formPago.fecha_pago,
      })
      await cargar(); setModalPago(false); setCuotaSeleccionada(null)
      if (resultado.comprobante) { setComprobanteGenerado(resultado.comprobante); setModalComprobante(true) }
    } catch (err) { alert(err.message) }
    finally { setGuardando(false) }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <Table>
        ...
        <Tbody>
          {cuotas.length === 0 ? (
            <tr><td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-400">No hay cuotas pendientes</td></tr>
          ) : cuotas.map((c) => (
            <tr key={c.id} className="hover:bg-gray-50/70 transition-colors">
              <Td><span className="font-medium text-gray-800">{c.alumnos?.apellido}, {c.alumnos?.nombre}</span></Td>
              <Td>{c.mes}</Td>
      ...
      <Modal isOpen={modalCuota} onClose={() => setModalCuota(false)} title="Generar Cuota">
        <form onSubmit={handleCrearCuota} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide">Mes *</label>
            <input type="month" value={formCuota.mes} onChange={(e) => setFormCuota(p => ({ ...p, mes: e.target.value }))} className={inputBase} />
          </div>
        ...
        <form onSubmit={handleRegistrarPago} className="space-y-4">
          {cuotaSeleccionada && (
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 text-sm">
              <p className="font-medium text-gray-800">{cuotaSeleccionada.alumnos?.nombre} {cuotaSeleccionada.alumnos?.apellido}</p>
              <p className="text-gray-400">Cuota: {cuotaSeleccionada.mes} — ${cuotaSeleccionada.monto}</p>
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-400">Número de comprobante:</span>
                <span className="font-bold text-gray-800 text-base">{comprobanteGenerado.numero}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-400">Fecha:</span>
                <span className="text-gray-700">{new Date(comprobanteGenerado.fecha).toLocaleDateString('es-AR')}</span>
              </div>
            </div>
